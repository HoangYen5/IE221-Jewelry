...
def list_units(user=Depends(checkPermission(["admin", "manager", "seller"]))):
    """Get a list of all units of measurement."""
    return {"errCode": 0, "data": unitService.list_units()}

@router.get("/search", status_code=200)
def search_units(q: str = "", user=Depends(checkPermission(["admin", "manager", "seller"]))):
    """Search units by keyword."""
    return {"errCode": 0, "data": unitService.search_units(q)}

@router.get("/next-code", status_code=200)
def get_next_code(user=Depends(checkPermission(["admin", "manager", "seller"]))):
...
def get_unit(unit_id: str, user=Depends(checkPermission(["admin", "manager", "seller"]))):
...
def create_unit(payload: UnitCreate, user=Depends(checkPermission(["admin", "manager"]))):
    """Create a new unit of measurement."""
    nid = unitService.create_unit(payload.model_dump())
    return {"errCode":0, "insertId": nid}

@router.put("/{unit_id}", status_code=200)
def update_unit(unit_id: str, payload: UnitUpdate, user=Depends(checkPermission(["admin", "manager"]))):
...
def delete_unit(unit_id: str, user=Depends(checkPermission(["admin", "manager"]))):
